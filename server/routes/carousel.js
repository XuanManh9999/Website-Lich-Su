...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      ...
      [quote, author || 'Thiên Sử Ký', image_url || null, parseInt(display_order) || 0, isActiveValue]
    );
...
router.put('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const [existing] = await db.query('SELECT id FROM carousel_slides WHERE id = ?', [id]);
    ...
    await db.query(
      ...
      [quote, author || 'Thiên Sử Ký', image_url || null, parseInt(display_order) || 0, isActiveValue, id]
    );
